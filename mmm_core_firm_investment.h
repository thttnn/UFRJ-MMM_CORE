...
*/
	...
	for (i=0; i<=(v[0]-1); i++)								//from 0 to investment period-1 lags
		{
		v[2]=VL("Firm_Effective_Orders", i);				//computates firm's effective orders of the current lag
	...
	for (i=v[0]; i<=(2*v[0]-1); i++)						//from investment period lag to twice investment period-1 lags
		{
		v[7]=VL("Firm_Effective_Orders", i);				//computates firm's effective orders of the current lag
	...
	v[9]=V("sector_expectations");                          //expectations parameter
...
*/
	...
	if(v[9]==1)														//if it is investment period for the firm
		...
		v[11]=V("sector_depreciation_period");						//depreciation period
		...
		CYCLE(cur, "CAPITALS")										//cycle trough all capital goods
		{
			...
			if(t<v[15] && v[15]<=(t+v[12]))							//if current capital good will depreciate in the next investment period (before new capital goods arrive)
	...
	if(v[3]==1)														// if it is investment period for the firm
	{
		...
		CYCLE(cur, "CAPITALS")										// CYCLE trought capital goods
		{
			...
			if((double)t!=v[19] && (double)t < v[9]+v[19]-v[1])		// if the capitalgood was not created in the current period nor will depreciate in the next investment period
				...
				v[23]=v[11]/(v[10]*((1/(v[17]))-(1/(v[4]))));		// calculates the payback
                 
				if(v[4]>v[17] && v[20]<=v[15] && v[23]<=v[13])		// if the cost of replacement is lower than current available funds and the paybakc calculus  is lower than the payback parameter
					{
