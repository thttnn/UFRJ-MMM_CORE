...
*/
	...
	if(v[2]==0)                               					//if the rest of the division is zero (adjust external income)
		{
		v[8]=V("external_income_sd");
		v[3]=VL("Annual_Growth", 1);							//nominal growth in the last year
		v[4]=norm(v[3], v[8]);								//random draw from a normal distribution with average equals to past growth and standard deviation equals to past growth in absolute value
